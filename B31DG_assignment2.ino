...
void setup(void)
{
  ...
  // rtc_wdt_disable();      // 禁用中断看门狗
  // rtc_wdt_protect_on();   // 重新开启看门狗写保护
  ...
  attachInterrupt(digitalPinToInterrupt(BUTTON_PIN), buttonPressedHandle, RISING);
   ...
   monitor.startMonitoring();
    ...
    // ticker2.attach_ms(3, task2);
    // ticker3.attach_ms(10, task3);
    // ticker4.attach_ms(10, task4);
    // ticker5.attach_ms(5, task5);
...
void loop(void) 
{
  ...
  // delay(10000);
    ...
    //   flag = false;  // 清除标志位
    //   // 执行需要的操作
    //   delayMicroseconds(250);  // 在主循环中使用延迟
    // }
    // else{
    //   flag = true;  // 清除标志位
    //   // 执行需要的操作
